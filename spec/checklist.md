...
- CI（2026-09-01）：GitHub Actions 四工作流（ci/scorecard/codeql/sonarcloud）+ Codecov 免 token 上传 + pkg.go.dev 自动抓取；README 双语徽章与结果链接就位；仅 SonarCloud 需 SONAR_TOKEN secret、Scorecard 提分需 master 分支保护。goreportcard 已于同日移除（官方宣布停止服务，其检查项由 CI 的 golangci-lint/go vet/gofmt 覆盖）
