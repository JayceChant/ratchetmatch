...
- [x] Task 23: WithSynonyms 同义词分组（feat/synonyms 分支，2026-09-04）。设计讨论定论（用户决策）：同义词是**整词级等价**而非 fold 的 rune 级轨道——词长/rune 序不同、trie 路径物理不可能合一（ε-转移 transducer 复杂度爆炸且扫描本就与词库规模无关，否决），实现形态必为「组员各自成词入 trie + 终止节点挂组号」；**分组是纯输出元数据，绝不改变匹配语义**（最左最长照旧裁决，胜者带自己的组）。用户三项决策：Match.Group 恒填充分区语义（未声明词自成单元素组，组号空间全函数）；提供 GroupWords(g) 访问器（密封接口新增方法不破坏兼容）；组员自动入库（组是一等输入）。
  - API：`New(kws, WithSynonyms(groups [][]string), WithCaseFold()?)`——两选项正交；`Match.Group int`（分区编号：声明组按声明序 0..k-1，未声明词按去重词库序接续单元素组）；`GroupWords(g)` 返回内部只读成员切片（越界 nil）。fold 模式词身份为折叠归一形（foldNorm = 逐 rune foldKey，与折叠 trie 边键同构——词库直接以归一形构建，自动机与原词输入等价；"PC"/"pc" 归一同形必然同组；GroupWords 返回归一形成员，ASCII 轨道代表为大写）。fold 下 Group 兼作规范词身份（Keyword 是大小写不定文本切片，按词计数无需再归一）。
  - 实现：resolveSynonyms（组员校验/组内去重/跨组冲突检测/词库扩充，New 先解析后构建、报错不构建）；词→组号 groupOf 表构建期定型 outGroups 平行数组（exact 沿 termGroup++fail 链拼接、与 outLens 同构共享切片；fold 的 flushFoldTerms 以 {nr,grp} 结构体排序置换后重建双数组，折叠同形词组号恒等去重任取）；pendHit+group 随链进出——**mergeCandidate 归并逻辑零改动，仅「同起点取更长」替换分支同步转移组号**；flushChain/appendAll 各 +1 次拷贝。machine 持 groups(声明成员表)/singletons(未声明词库下标表)/words(去重词库)——无 WithSynonyms 时 groups/singletons 为 nil、words 单切片，GroupWords 后段经下标切片零额外分配。
  - 兼容性：Match 追加字段（unkeyed 字面量已全部改写为 keyed）；Matcher 接口新增 GroupWords（密封接口红利）；New 词库可仅由组构成（New(nil, WithSynonyms(...))），keywords 与 groups 皆空才报 empty。
  - 测试：既有测试全量改写带组号（naiveSearch oracle、foldOracleAll、三基线参照 interval.kw+groups 表——TestBaselineEquiv 的 DeepEqual 全等由 identityGroups 锁定）；新增 synonyms_test.go：分区语义/GroupWords 往返/纯组词库、语义正交（同词库有/无分组 FindAll+Overlapping+FindNext 对照 + 链规则边角「电脑城胜电脑」「必死候选不弹链」）、200 组随机双 oracle、fold 正交（归一同组/跨组冲突/语义不变）、校验报错（空组/组员非法/跨组冲突含 fold 归一形）、并发 -race。
  - 验证：gofmt/vet/golangci-lint（0 issues）/test/-race 全过；fuzz 20s 18.5 万 execs 零失败；基准无回退（FindAll 中文 1.95ms/混合 1.05ms/首停 97µs，allocs 12/10/2 与历史一致；New100 652 allocs 系测试内 identityGroups 与对照口径变化，非库代码增量）。
- [x] Task 24: 组访问器更名与全量返回（feat/synonyms 分支，2026-09-04，用户要求）：`GroupWords(g)` 更名 **`WordGroup(g)`**（命名对齐 `WithSynonyms` 语义——按词取组），并新增 **`WordGroups() [][]string`** 按组号升序一次返回全部组。实现：WordGroup 语义不变（内部只读切片、越界 nil）；WordGroups 外层切片为本次新建可自由重排、元素复用内部只读切片（零逐组拷贝——无 WithSynonyms 时为 len(words) 个 `words[i:i+1]` 视图，仅一次外层分配）；machine.groups==nil（未声明组）与 != nil（声明组+singletons 视图）两分支。全仓引用同步（接口/实现/option 与 build 注释/README 双语/spec 需求与场景/synonyms_test）；测试补 WordGroups：与 WordGroup 逐组一致、无 WithSynonyms 词库的单元素组序列、外层重排不影响内部状态、fold 归一形成员。勿再以「与 Match.Group 命名一致性」为由改回 Group* 前缀。
- [x] Task 25: example/ 新增同义词示例并回顾既有示例（用户要求，feat/synonyms 分支）：新增 `example/synonyms/`（示例 5）——WithSynonyms 声明组 + 组员自动入库 + 单元素组接续编号 + Match.Group 逐命中输出 + WordGroups/WordGroup 按组聚合 + WithCaseFold 正交（PC/pc/个人电脑 归一同组）；`wordcount/` 增补「按同义词组聚合」一节——统计场景是分组的最直接受益者（分散词频 → 概念级计数，命中自带组号后聚合即切片自增）；`basic/` 尾注补指向 synonyms 的指针；`semantics/`、`iterate/` 与分组无语义交集（前者演示最左最长本身、后者演示流式迭代），保持不动。doc.go 与双语 README 的示例目录清单同步加 `./synonyms/`。验证：example 模块 go build/vet/gofmt/golangci-lint 全过，synonyms 与 wordcount 逐个 go run 输出核对无误（组号 2/3 为单元素组接续、组0 ×3 聚合、fold 三形态同组）；主模块 test 不受影响。
