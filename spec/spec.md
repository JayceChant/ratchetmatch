...
- 候选仍按结束位置升序到达（同一结束位置按长度降序），经链规则归并：
  - 候选起点 < 链尾起点：候选更左，**弹出链尾**（被覆盖），继续向前比较；链空后入链
  - 候选起点 == 链尾起点：**取更长**（替换或维持链尾）
  - 候选起点 >= 链尾结束位置（不重叠）：**入链**接续
  - 其余（与链尾重叠且起点更晚）：候选被链尾遮蔽，丢弃
...
#### Scenario: 断词后短词结算
- **WHEN** 词库 {"国", "人", "中国人"}，文本 "中国人x"
- **THEN** 输出 "中国人"(Start=0, End=9)（"x" 断词后链结算；此场景链中 "国" 先被更左的 "中国人" 覆盖）
...
- 扫描语义（最左最长）与 FindAll 完全一致；对同一 text，FindAll 的结果等价于从 offset=0 开始反复 FindNext 并以 End 推进
