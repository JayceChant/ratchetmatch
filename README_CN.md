...
与三类语义等价的参照基线对比（50 关键词 × 约 10 万 rune 文本；参照实现与正式 API 的等价性由守卫测试锁定，见 `bench_test.go`）：
...
| 逐关键词 strings.Index 全文扫一轮 | 0.67 ms | ~7.6x |

要点：逐词基线（BM / strings.Index）随词库线性变慢（每关键词各扫全文一遍），词库越大差距越大；纯 Trie 单遍扫描与词库大小无关，但共享词库文本下 fail 链对多前缀共尾的收益有限。`strings.Index` 已是标准库 SIMD 加速的最快单串搜索，手写逐字节双循环只会白白丢掉向量化（分析结论，见 `bench_test.go`）。数值为量级参考，随硬件与数据分布浮动。
