...
| 标识 | 说明 |
|---|---|
| `New(keywords []string) (*Matcher, error)` | 构建不可变 `Matcher`。词库为空、含空字符串、含非法 UTF-8 字节或含 U+FFFD（替换字符）返回可区分的错误；重复关键词自动去重 |
...
| 周期性词库无空档 | `{"0", "000"}` / `"000000000001"` | `000`、`000`、`000`、`0`、`0` |

规则要点：从左到右，起点最小者优先；同一起点取完整出现的最长关键词（长词进行中优先延续，断词才结算短词）。命中之间互不重叠、不留空档（被遮蔽的候选不会挤掉本可提交的命中），每个文本位置至多属于一个命中；同一关键词同一位置至多输出一次。
